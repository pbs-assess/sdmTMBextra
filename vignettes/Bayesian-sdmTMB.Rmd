...
pkgs <- ggplot_installed && inla_installed && tmbstan && rstan && bayesplot
...
For this, we can make use of `sdmTMB::simulate.sdmTMB()` while passing in our Stan model to the `mcmc_samples` argument.
`sdmTMB::simulate.sdmTMB()` will take draws from the joint parameter posterior and add observation error.

```{r, eval=FALSE}
set.seed(19292)
s <- simulate(fit_mle, mcmc_samples = sdmTMBextra::extract_mcmc(fit_stan), nsim = 50)
bayesplot::pp_check(
  sim_dat$observed,
  yrep = t(s),
...
We can make predictions with our Bayesian model by supplying it to the output of `extract_mcmc` to the `mcmc_samples` argument in `predict.sdmTMB()`.

```{r}
pred <- predict(fit_mle, mcmc_samples = sdmTMBextra::extract_mcmc(fit_stan))
```
...
pred <- predict(fit_mle, newdata = nd, mcmc_samples = sdmTMBextra::extract_mcmc(fit_stan))
...
fit_pred <- predict(
  fit_mle,
  newdata = nd,
  mcmc_samples = sdmTMBextra::extract_mcmc(fit_stan),
  sims_var = "omega_s" #<
)
